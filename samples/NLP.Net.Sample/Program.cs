...
namespace NLP.Net.Sample
{
    class Program
    {
        ...
        static void japaneseNlp()
        {
            string text = @"新職来るし遅くてももう２～３ヶ月もすれば再販されるはず

あと学者は先駆者９ + ジェネレーター、猛攻とかくらいしか見ないので
  極度の人選レイドじゃないかぎり入れてもらえるはず

自分は一応裏面グレーシャつけてますけど
スイッチするとPTメンバーにも負荷かかるので
一度もスイッチしないでレイドいってます";

            var result = POSExtractor.Extract(NLPType.Japanese, text);
            foreach (var item in result)
            {
                Console.WriteLine(JsonConvert.SerializeObject(item));
            }
